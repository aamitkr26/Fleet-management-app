...
import { GraphicalFuelDialog } from '../GraphicalFuelDialog';
import { 
  Droplet, 
  Download,
...
export function FuelReports() {
  ...
  const fuelRefillData = [
    ...
    },
    {
      ...
      driver: 'Vikram Singh',
      refillDate: yesterdayStr, // Yesterday
      refillTime: '08:00 AM',
      fuelRefilled: 55,
  ...
  const calculate30DayStats = (vehicleNumber: string) => {
  ...
  const vehicleStats = fuelRefillData.reduce((acc: any, record) => {
      ...
      <Card className="border-slate-200">
        ...
        <CardContent>
          <Table>
            ...
            <TableBody>
              {vehicleStatsArray.map((vehicle: any) => (
