...
interface GraphicalFuelDialogProps {
  open: boolean;
...
const vehicleData: Record<string, {
  name: string;
  hasAnomaly: boolean;
  dataPoints: { time: string; fuel: number }[];
  anomalyDetails?: {
    time: string;
    fuelDropped: number;
    status: string;
...
export function GraphicalFuelDialog({ open, onOpenChange, vehicleNumber }: GraphicalFuelDialogProps) {
  ...
  const drawFuelChart = () => {
    ...
    const sloshingPoints: { x: number; y: number }[] = [];
